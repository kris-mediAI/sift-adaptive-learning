...
# Capture concept evidence before completing the dynamic task.
...
if concept_before_completion is None:
    raise SystemExit(
        "FAIL: task concept was not found before completion."
    )
...
# A correct answer at the current mastery level is
# confirming evidence. Mastery does not have to increase.
if updated_concept["attempts"] <= before_attempts:
    raise SystemExit(
        "FAIL: assessment attempt was not recorded."
    )

if updated_concept["correct_attempts"] <= before_correct_attempts:
    raise SystemExit(
        "FAIL: correct evidence was not recorded."
    )

if updated_concept["last_score"] != completed_assessment["score"]:
    raise SystemExit(
        "FAIL: latest assessment score was not recorded."
    )

if not 0.0 <= updated_concept["mastery"] <= 100.0:
    raise SystemExit(
